...
var files = fs.readdirSync('/Volumes/Troper/vyperAuthorFilesGenerated/');
...
function prepFile(index){
	if (index != byteFiles.length){
		...
		fs.readFile('/Volumes/Troper/vyperAuthorFilesGenerated/' + byteFiles[index], 'utf-8', function(err, data){
			entry.push(data.slice(0, data.length-1));
			fs.readFile('/Volumes/Troper/vyperAuthorFilesGenerated/' + abiFiles[index], 'utf-8', function(err, data2){
